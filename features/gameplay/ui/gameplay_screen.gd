...
signal equipment_enhancement_intent_requested(
	uid: String
)
...
# =========================================================
# DEBUG — ENHANCEMENT E2E
# =========================================================
#
# Herramienta exclusiva de Development.
#
# Ctrl + Shift + H:
#
# - busca una instancia Equipment autoritativa;
# - prioriza Inventory;
# - si no encuentra, intenta Equipment equipado;
# - emite únicamente el UID.
#
# NO decide:
#
# - container autoritativo;
# - Enhancement actual;
# - Enhancement siguiente;
# - requirements;
# - persistencia.
#
# Todo eso sigue perteneciendo al Game Server.
# =========================================================
...
func _unhandled_key_input(
	...
	if _request_debug_equipment_enhancement():
		get_viewport().set_input_as_handled()


# =========================================================
# DEBUG — SOLICITAR ENHANCEMENT
# =========================================================

func _request_debug_equipment_enhancement() -> bool:
	if not OS.is_debug_build():
		return false
	...
	if player_state == null:
		print(
			"GameplayScreen | DEBUG Enhancement sin PlayerRuntimeState."
		)
	...
	var item := (
		_find_debug_enhancement_candidate()
	)


	if item == null:
		print(
			"GameplayScreen | DEBUG Enhancement sin Equipment disponible."
		)
	...
	var observed_container := "equipment"


	if (
		player_state.inventory != null
		and
		player_state.inventory.items.has(
			item
		)
	):
		observed_container = "inventory"


	var observed_level := int(
		item.state.get(
			"enhancement_level",
			0
		)
	...
	print(
		"GameplayScreen | DEBUG Enhancement solicitado",
		" | UID: ",
		uid,
		" | Item: ",
		item_id,
	...
	equipment_enhancement_intent_requested.emit(
		uid
	)
...
# =========================================================
# DEBUG — BUSCAR CANDIDATO
# =========================================================

func _find_debug_enhancement_candidate() -> ItemInstance:
	if player_state == null:
		return null
	...
	# 1. INVENTORY
	#
	# Lo priorizamos porque poseer un item no implica
	# cumplir todavía sus Usage Requirements.
	# -----------------------------------------------------

	if player_state.inventory != null:
		for item: ItemInstance in player_state.inventory.items:
			if item == null:
				continue
			...
			if not item.definition.is_equipment():
				continue
	...
	# 2. EQUIPMENT ACTUAL
	#
	# Si no existe Equipment en Inventory, probamos una
	# instancia equipada. El Game Server volverá a validar
	# Usage Eligibility para el candidato +(N+1).
	# -----------------------------------------------------

	if player_state.equipment != null:
		var equipped_items := (
			player_state.equipment.get_equipped_items()
		)


		for item_value: Variant in equipped_items.values():
			if not (
